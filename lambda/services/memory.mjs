...
const SESSION_IDLE_HOURS = parseFloat(process.env.SESSION_IDLE_HOURS || '2')
...
export async function getContext(userId) {
  ...
  const rawItems = turnsRes.Items || []

  // sk format: TURN#<iso>#role — rawItems is newest-first
  let isStaleSession = false
  if (rawItems.length > 0) {
    const latestIso = rawItems[0].sk.S.split('#')[1]
    const idleMs = Date.now() - new Date(latestIso).getTime()
    isStaleSession = idleMs > SESSION_IDLE_HOURS * 3600 * 1000
  }

  // reverse to chronological for prompt building
  const recentTurns = rawItems
    .slice()
    .reverse()
    .map((item) => ({
      role: item.role.S,
  ...
  return { summary, recentTurns, isStaleSession }
}
...
/**
 * Compress ALL remaining turns into the rolling summary and delete them.
 * Used on idle-session boundary so the next conversation starts fresh
 * while preserving long-term context inside the summary.
 */
export async function archiveSession(userId, provider) {
  const pk = `USER#${userId}`
  ...
  const allTurns = allItems.map((item) => ({
    role: item.role.S,
    content: item.content.S,
  }))
  ...
  const newSummary = await provider.summarize(existingSummary, allTurns)
